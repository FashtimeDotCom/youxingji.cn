...
(function($){
	$.fn.Result = function(options) {
			...
			.keyup(function(e) {
				...
				switch(e.keyCode) {
					...
					default:
						...
						timeout = setTimeout(function(){_onChange();}, options.delay);
						break;
				}
			})
			.focus(function(){
		...
		{
			$_div.hide().css({"position":"absolute",/*"height":options.showHeight,*/"z-index":"9999"})
		...
		function findPos(obj) 
		{
			...
			var width = parseInt($(obj).width())+5;
			var height = parseInt(curtop)+parseInt($(obj).height()) + 6;
			return {x:curleft,y:height,w:width};
		}
		
		function _showDiv() {
			var pos = findPos(rethis);
			var iWidths = (pos.w > 0) ? pos.w : options.showWidth;
			$_div.css({
				width: (parseInt(iWidths) + 3) + "px",
				top: (pos.y + 6) + "px",
				left: pos.x + "px"
			}).show();
		};
		...
		function receiveData(keyword, data) 
		{
			if (data) 
			{
				...
				_div.appendChild(createDom(data));
				
				if( options.autoFill && ($(rethis).val().toLowerCase() == keyword.toLowerCase()) ) autoFill(data[0][0]);
				_showDiv();
			} 
			else 
			{
		...
		function getResult(keyword) 
		{
			...
			{
				$.getJSON(makeUrl(keyword), function(result) {
					data = parseData(result.info);
					receiveData(keyword, data);
				});
			} 
			else 
			{
		...
		function makeUrl(keyword) {
			var url = SITE_URL + options.url + "/keyword/" + keyword;//escape(keyword);
			var exparam = '';
			for (var i in options.extraParams) {
				exparam += "/" + i + "/" + escape(options.extraParams[i]);
			}
			return url + exparam;
		};
