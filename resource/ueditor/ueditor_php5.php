...
class UEditor{
    ...
    public function editor($name, $value = "", $config = array(), $events = array())
	{
        $out = "";
        if (!$this->initialized) {
			$out .= $this->init($config['noscript']);
		}
        $out .= '<div id="' . $name . '" style="width:' . $config['minWidth'] . 'px; margin:5px 0px 5px 0; text-indent:0;"></div>';
        ...
        $str .= "var " . $name . "Editor = baidu.editor.ui.Editor(options" . $name . ");";
    ...
    private function init($noscript)
	{
		static $initComplete;
		$out = "";
		...
		if(!$noscript)
		{
			//$out .= "<script type=\"text/javascript\" src=\"" . $ueditorPath . 'ueditor.config.js' . $args . "\"></script>\n";
			//$out .= "<script type=\"text/javascript\" src=\"" . $ueditorPath . 'ueditor.all.min.js' . $args . "\"></script>\n";
