<?php /* vpcvcms compiled created on 2018-09-14 10:12:46
...
" />
	...
	<style>
		...
		.myfile .buttonTier .lump{width: 100%;margin-top: 16%;}
		...
		.maskLayer{position: absolute;top: 0;left: 0;right: 0;bottom: 0;z-index: 3;}
		...
		@media only screen and (max-width: 414px) {
			...
			.myfile .buttonTier .lump{padding-left: 20%;margin-top: 19%;}
...
" id="title" placeholder="请在这里输入游记的标题">
						...
						<div class="tit">
							<input type="text" class="inp" value="<?php echo $this->_tpl_vars['res']['title']; ?>
" id="describe" placeholder="请在这里输入游记的摘要/封面描述">
						</div>
						<div class="pic-video tit">
							<div class="myfile fix">
								<img class="note_bg" src="/resource/m/images/user/note_bg.jpg"/>
								...
								<div class="maskLayer" id="chooseImage"></div>
							</div>
						</div>
...
" id="tag" placeholder="请输入标签(可选)，每个标签最多四个字，如：旅游知识/美食，用正斜杠分开">
						...
						<div class="tit">
							<textarea class="layui-textarea" id="LAY_demo1" style="display: none" placeholder="请发表你的游记"><?php echo $this->_tpl_vars['res']['content']; ?>
</textarea>
						</div>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			function add (con) {
				...
				if(imgUrl == "http://test.youxingji.cn/resource/m/images/user/note_bg.jpg") {
				//if(imgUrl == "http://m.youxingji.cn/resource/m/images/user/note_bg.jpg") {
				//if(imgUrl == "http://localhost.youxingji.cn/resource/m/images/user/note_bg.jpg") {
					layer.msg('请上传封面图片');
					return false;
				}
				if(!con) {
			...
			function draft(con) {
				...
				var tag = $('#tag').val();//标签
				address = $('#address').val();//定位
				if(!title && !describe && imgUrl == "http://test.youxingji.cn/resource/m/images/user/note_bg.jpg" && !con) {
				//if(!title && !describe && imgUrl == "http://m.youxingji.cn/resource/m/images/user/note_bg.jpg" && !con) {
				//if(!title && !describe && imgUrl == "http://localhost.youxingji.cn/resource/m/images/user/note_bg.jpg" && !con) {
					layer.msg('不能全为空');
					return false;
				}
...
".split(',');
			var index = {
				...
				render: function() {
					var me = this;
					me.chooseImage = $('#chooseImage');//选择图片 并上传
				...
				_chooseImage: function(e) {
					...
					wx.chooseImage({
						...
						success: function(res) {
							...
							var load = layer.load();
							var src = "//" + me.datas.host + '/' + 'index.php?m=api&c=index&v=uploadpic';
							var data = {
							  	...
							  	success: function (res) {
							    	...
							    	$.ajax({
										...
										success: function(res) {
											...
											$(".note_bg").attr("src",localIds);
											$(".buttonTier").addClass("dis_none");
										},
									  	fail: function (res) {
									  		alert(res);
									  	}
