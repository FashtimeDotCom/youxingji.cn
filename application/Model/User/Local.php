...
class Model_User_Local
{
    ...
    {
		$uids = Core_Comm_Util::formatName2Array($uids);
    ...
    private static function _fetUsersInfo ($uids)
    {
        ...
        //获取本地用户信息
        $userInfo = Model_User_Member::getUserInfosByUids ($uids);
        $returnUser = array();
		
        //把某些属性替换成本地内容
        foreach ($uids as $val)
        {
            $key = empty(self::$_userID2Key[$val])?Core_Comm_Util::getUserCachePath($val, self:: $_userLocalDir):self::$_userID2Key[$val];//此用户要缓存的key
