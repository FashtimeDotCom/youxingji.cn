 ...
 * @author snake.L
 */
class Model_User_Member extends Core_Model
{
    /**
     * 表名
     *
    ...
    public function editUserInfo ($userInfo)
    {
        ...
        !empty ($userInfo['uid']) && Model_User_Local::delCache ($userInfo['uid']);
        return $update;
    }
    ...
    public static function getUserInfosByUids ($uids)
    {
        if (is_array ($uids) && count ($uids) > 0) {
            $sql = 'SELECT a.* FROM `##__user_member` AS a WHERE a.uid IN(' . Core_Comm_Util::array2string ($uids) . ')';
	...
	public function formatUserLevel($uid)
	{
		...
		$_groupModel = new Model_User_Group();
