 ...
 * 用户地址模型
 */
class Model_User_Address extends Core_Model
{
	/**
	 * 表名
	 * 
	...
	public function getDefaultAddressesByUid($uid)
	{
		...
		$address['location'] = Core_Comm_Util::getLocation($address['province'], $address['city'], $address['district']);
		return $address;
	}
	...
	public function getAddressList($whereArr=array (), $orderByArr=array (), $limitArr=array ())
	{
		...
		{
			$addresses[$idx]['location'] = Core_Comm_Util::getLocation($address['province'], $address['city'], $address['district']);
			$addresses[$idx]['username'] = C::M('User_Member')->getInfoByUid('username', $address['uid']);  
		}
		return $addresses;
	}
}
