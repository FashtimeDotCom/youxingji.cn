 ...
 * 论坛主题模型
 */
class Model_Forumthread extends Core_Model
{
	/**
     * Database table name.
     * @var (string)
	...
	{
		$now = Core_Fun::time();
    	$prev = Core_Fun::strtotime("-1 day");

    	$nowstime = Core_Fun::strtotime(Core_Fun::date('Y-m-d', $now) . ' 0:0:0');
    	$nowetime = Core_Fun::strtotime(Core_Fun::date('Y-m-d', $now) . ' 23:59:59');

    	$prevstime = Core_Fun::strtotime(Core_Fun::date('Y-m-d', $prev) . ' 0:0:0');
    	$prevetime = Core_Fun::strtotime(Core_Fun::date('Y-m-d', $prev) . ' 23:59:59');
