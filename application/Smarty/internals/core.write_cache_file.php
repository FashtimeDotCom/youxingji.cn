...
function smarty_core_write_cache_file($params, &$smarty)
{
	...
	/*YUI*/
	$rule = "/<!--[\"|'| ]{0,}([^>]*)-->/";
	if(preg_match_all($rule, $params['results'], $array))
	{
		foreach($array[1] AS $idx => $value)
		{
			$params['results'] = str_replace('<!--' . $value . '-->', '', $params['results']);
		}
	}
	$params['results'] = str_replace(array("\r\n", "\r", "\n", "\t"), '', $params['results']);
	$pattern=array("/\s+/");
	$replace = array(" ");
	$params['results'] = preg_replace($pattern, $replace, $params['results']);
