...
class Smarty_Compiler extends Smarty {
    ...
    function _compile_file($resource_name, $source_content, &$compiled_content)
    {
        ...
        $source_content = preg_replace_callback($search, function($match){
        ...
        $template_header = "<?php /* vpcvcms compiled created on ".strftime("%Y-%m-%d %H:%M:%S")."\n";
    ...
    function _compile_tag_args($template_tag)
    {
        ...
        return array('tag_command' => $match[1], 'tag_modifier' => isset($match[2]) ? $match[2] : null, 'tag_args' => isset($match[3]) ? $match[3] : null);
    }
    ...
    {
        /* Matched comment. */
        $_tag_args = $this->_compile_tag_args($template_tag);
        $tag_command = $_tag_args['tag_command'];
        $tag_modifier = $_tag_args['tag_modifier'];
        $tag_args = $_tag_args['tag_args'];
        ...
        switch ($tag_command) {
            ...
            case 'vplist':
                $this->_push_tag('vplist');
                return $this->_compile_vplist_start($tag_args);
                break;
            
            case '/vplist':
                $_open_tag = $this->_pop_tag('vplist');
                return "<?php endforeach; endif; unset(\$_from); ?>";
                break;
            case 'vpconfig':
                return $this->_compile_vpconfig_tag($tag_args);
                break;
            case 'vpadsingle':
                return $this->_compile_vpadsingle_tag($tag_args);
                break;
            case 'strip':
            case '/strip':
     ...
     * @return string
     */
    function _compile_vplist_start($tag_args)
    {
        $attrs = $this->_parse_attrs($tag_args);
        $arg_list = array();
        foreach($attrs AS $key => $attr)
        {
            if($key != 'from' && $key != 'item' && $key != 'name' && $key != 'key')
            {
                if(preg_match('/\$/', $attr))
                {
                    $attr = $this->_dequote($attr);
                    $_attrs = explode('.', str_replace('$', '', $attr));
                    if(count($_attrs) > 1)
                    {
                        $arg_list[] = "'" . $key . "'" . " => " . "\$this->_tpl_vars['" . $_attrs[0] . "']['" . $_attrs[1] . "']";
                    }
                    else
                    {
                        $arg_list[] = "'" . $key . "'" . " => " . "\$this->_tpl_vars['" . $_attrs[0] . "']";
                    }
                }
                else
                {
                    $arg_list[] = "'" . $key . "'" . " => " . "'" . $this->_dequote($attr) . "'";
                }
                //$arg_list[] = "'" . $key . "'" . " => " . "'" . $this->_dequote($attr) . "'";
            }
        }
        ...
        $output = '<?php ';
        $output .= "\$_from = C::T('" . $from . "')->getList(array($arg));";
    ...
    /**
     * Compile {vpconfig ...} tag.
     *
     * @param string $tag_args
     * @return string
     */
    function _compile_vpconfig_tag($tag_args)
    {
        $attrs = $this->_parse_attrs($tag_args);
        if (empty($attrs['key'])) {
            return $this->_syntax_error("foreach: missing 'key' attribute", E_USER_ERROR, __FILE__, __LINE__);
        }
        ...
        if (empty($attrs['group'])) {
            return $this->_syntax_error("foreach: missing 'group' attribute", E_USER_ERROR, __FILE__, __LINE__);
        }
        $group = $this->_dequote($attrs['group']);
        if (!preg_match('~^\w+$~', $group)) {
            return $this->_syntax_error("foreach: 'item' must be a variable name (literal string)", E_USER_ERROR, __FILE__, __LINE__);
        }
        
        if (isset($attrs['default'])) {
            $default  = $this->_dequote($attrs['default']);
        } else {
            $default = '';
        }
        
        $output = '<?php ';
        $output .= "echo Core_Config::get('" . $key . "', '" . $group . "', '" . $default . "');";
        $output .= '?>';
    ...
    /**
     * Compile {vpadsingle ...} tag.
     *
     * @param string $tag_args
     * @return string
     */
    function _compile_vpadsingle_tag($tag_args)
    {
        $attrs = $this->_parse_attrs($tag_args);
        if (empty($attrs['tagname'])) {
            return $this->_syntax_error("foreach: missing 'tagname' attribute", E_USER_ERROR, __FILE__, __LINE__);
        }

        $tagname = $this->_dequote($attrs['tagname']);

        $typeid = $this->_dequote($attrs['typeid']);

        if(!empty($typeid))
        {
            $_typeid = explode('.', str_replace('$', '', $typeid));
            if(count($_typeid) > 1)
            {
                $typeid = "\$this->_tpl_vars['" . $_typeid[0] . "']['" . $_typeid[1] . "']";
            }
        }
        
        $output = '<?php ';
        $output .= "echo Core_Fun::getAdSingle('" . $tagname . "', " . $typeid . ");";
        $output .= '?>';
