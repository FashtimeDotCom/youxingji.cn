...
class Core_Template
{
    ...
    protected static function getSmarty()
    {
        if(null === self::$SMARTY)
        {
            ...
            $smarty = new Smarty();
			$model = Core_Controller_Front::getInstance()->getModelName();
            //一些设置
			...
			$iscaching = Core_Config::get('site_cache', 'site', false);
			if($iscaching)
			{
				if($model != 'admin')
				{
    ...
    public static function dealSmarty($smarty, $resource_name, $compile_id , $seo)
    {
        ...
        if($seo)
        {
            $_smarty_results = preg_replace_callback("!action=\"/([^\"]*)\"!iU", function($match){
                return 'action="' . Core_Template::seoChange($match[1]) . '"';
            }, $_smarty_results);
            $_smarty_results = preg_replace_callback("!href=\"/([^\"]*)\"!iU", function($match){
                return 'href="' . Core_Template::seoChange($match[1]) . '"';
            }, $_smarty_results);
            $_smarty_results = preg_replace_callback("!href=\'/([^\']*)\'!iU", function($match){
                return 'href="' . Core_Template::resourceChange($match[1]) . '"';
            }, $_smarty_results);
            $_smarty_results = preg_replace_callback("!src=\"/([^\"]*)\"!iU", function($match){
                return 'src="' . Core_Template::resourceChange($match[1]) . '"';
            }, $_smarty_results);
        }
    ...
    public static function seoChange($url)
    {
		...
		$hostname = Core_Comm_Util::getHostname();
		$domin = Core_Comm_Util::getDomin();
