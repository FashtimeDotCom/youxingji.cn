...
class Core_Comm_Util
{
    ...
    {
        $location = '';
        isset($provinceIndex) && $location = C::M('region')->getRegionName($provinceIndex) . ' ';
        isset($cityIndex) && $location .= C::M('region')->getRegionName($cityIndex);
        isset($districtIndex) && $location .= ' ' . C::M('region')->getRegionName($districtIndex);
		isset($roadIndex) && $location .= ' ' . C::M('region')->getRegionName($roadIndex);
    ...
    {
        if(empty($uid)) throw new Core_Exception('没有用户错误');
	...
	public static function httpDownPic($link)
	{
		...
		{
			$date = 'uploadfile/attach/day_' . Core_Fun::date("Ymd");
			$name = Core_Fun::date("YmdHis") . mt_rand(1000,9999);
			...
			return $attach = Core_Fun::getWebroot() . $date . "/" . $name . $itype;
		}
	}
