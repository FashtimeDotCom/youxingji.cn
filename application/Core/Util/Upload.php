...
class Core_Util_Upload
{
	...
	public static function upload( $inputFiled , $urlExt = '', $allowExt=FALSE, $maxsize=FALSE, $thumb=TRUE, $water = TRUE)
	{
		...
		else
		{
			...
			if( in_array(strtolower($extension), explode(',', self::EXT)) )
			{
				...
				{
					$waterenable = Core_Config::get('enable', 'water', '0');
					...
					{
						//生成缩略图
						self::getThumbPic($target, Core_Config::get('thumb_width', 'basic', '240'), Core_Config::get('thumb_height', 'basic', '180'), $urlExt);
						$rt['thumb'] =  $_webroot . self::thumbUrl . $urlExt . "/image/". Core_Fun::date('Ymd') . '/' . $filename;
					}
					if($water)
					{
						if($waterenable)
						{
							...
							self::setWater(HTDOCS_ROOT . self::thumbUrl . $urlExt . "/image/". Core_Fun::date('Ymd') . '/' . $filename);//缩略图水印
						}
					}
				}
			}
	...
	public static function webuploader($fileField, $config, $thumb=FALSE, $water = FALSE)
    {
		...
		if (!$file) {
            ...
            return array('code'=>0,'msg'=>self::$stateInfo);
        }
        if (self::$file['error']) {
			...
			if (!(move_uploaded_file($file["tmp_name"], self::$filePath) && file_exists(self::$filePath))) { //移动失败
                ...
                if( $flag ){//大于0.5M
                    ...
                    $image_info = getimagesize(self::$filePath);
                    $width=$image_info[0];
                    $height=$image_info[1];
                    ...
                    $image_class=new Core_Lib_Image(self::$filePath,$percent);
                    $image_class->compressImg(self::$filePath);
                }
			...
			if($thumb)
			{
				self::getThumbPic(self::$filePath, Core_Config::get('thumb_width', 'basic', '240'), Core_Config::get('thumb_height', 'basic', '180'), '/' . self::$config['ext']);
				//生成缩略图
				$ext = self::$config['ext'] ? '/' . self::$config['ext'] : '';
				$thumburl =  $_webroot . self::thumbUrl . $ext . '/image/' . Core_Fun::date('Ymd') . '/' . self::$fileName;
			}
			if($water)
			{
				if($waterenable)
	...
	{
		$markImg = Core_Config::get('markimg', 'water', 'resource/images/water.png');
		$markText = Core_Config::get('marktext', 'water', 'HST');
		$markPos = Core_Config::get('markpos', 'water', '0');
		$fontSize = Core_Config::get('fontsize', 'water', '12');
		$markType = Core_Config::get('marktype', 'water', 'text');
		$markDiaphaneity = Core_Config::get('diaphaneity', 'water', '90');
		$TextColor = '58,11,165';
		...
		if(!strcmp($markType,"text"))
		{
			...
			$waterText = Core_Fun::iconv('GB2312', 'UTF-8', $markText);
		...
		if( $image == '' )
		{
			...
			if( !$info ){
				...
				$ret['result_des'] = array(
					"big"   => str_replace(HTDOCS_ROOT, Core_Fun::getWebroot(), $thumbname01),
					"middle"=> str_replace(HTDOCS_ROOT, Core_Fun::getWebroot(), $thumbname02),
					"small" => str_replace(HTDOCS_ROOT, Core_Fun::getWebroot(), $thumbname03),
					"normal" => Core_Fun::getWebroot() . $img
				);
			}
		}
		return $ret;
