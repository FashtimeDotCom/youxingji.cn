...
class Core_Model
{
        ...
        if($value != ''){
            if(!$this->_where){
                if($suffix == 'find_in_set'){
                    $this->_where = "FIND_IN_SET('" . Core_Db::sqlescape($value) . "', " . $where . ")";
                } elseif($suffix == 'like'){
                    $this->_where = $where . " LIKE '%" . Core_Db::sqlescape($value) . "%'";
                } elseif($suffix == 'in'){
                    $this->_where = $where . " IN (" . Core_Db::sqlescape($value) . ")";
                } else{
                    $this->_where = $where . $suffix . "'" . Core_Db::sqlescape($value) . "'";
                }
            }else{
                if($suffix == 'find_in_set'){
                    $this->_where .= " " . $linkfix . " FIND_IN_SET('" . Core_Db::sqlescape($value) . "', " . $where . ")";
                } elseif($suffix == 'like'){
                    $this->_where .= " " . $linkfix . " " . $where . " LIKE '%" . Core_Db::sqlescape($value) . "%'";
                } elseif($suffix == 'in'){
                    $this->_where .= " " . $linkfix . " " . $where . " IN (" . Core_Db::sqlescape($value) . ")";
                } else{
                    $this->_where .= " " . $linkfix . " " . $where . $suffix . "'" . Core_Db::sqlescape($value) . "'";
                }
            }
        }else{
            $this->_where = $where;
    ...
    public function select(){
        ...
        return Core_Db::fetchAll($sql, $limits[1], $limits[0]);
    }
    ...
    public function add ($add = array(), $safe=array (), $replace=false){
        if($replace){
            $rt = Core_Db::replace ($this->_tableName , $add , $safe);
        }else{
            $rt = Core_Db::insert ($this->_tableName , $add , $safe);
        }
    ...
    public function update ($update = array()){
        ...
        $rt = Core_Db::update ($this->_tableName , $where, $update);
        $this->_where = '';
        return $rt;
    }
    ...
    public function updateall ( $where = '' , $update = array(), $unset = array()){
        ...
        $rt = Core_Db::update ($this->_tableName , $where, $update, array(), $unset);
        $this->_where = '';
        return $rt;
    }
    ...
    public function delete (){
        ...
        Core_Db::execute ("Delete From `{$this->_tableName}` where {$where}" , true);
        $this->_where = '';
        return true;
    }
    ...
    public function queryAll ($whereArr=array (), $orderByArr=array (), $limitArr=array (), $fileds=array()){
        ...
        //echo $sql;
        return Core_Db::fetchAll($sql, $row_count, $offset);
    }
    ...
    public function formatWhere($whereArr)
    {
        ...
        if(!empty($whereArr))
        {
            foreach ((array)$whereArr as $value)
            {
                ...
                {
                    $where .= (empty($where)?' WHERE ':' AND ') . $value[2] . $prefix . "'" . Core_Db::sqlescape($value[1]) . "'," . $value[0] . $suffix;
                }
                else if(!empty($value[2]) && strtoupper($value[2]) == 'IN')
                {
                    $where .= (empty($where)?' WHERE ':' AND ') . $value[0] . " " . $value[2] . " " . $prefix . Core_Db::sqlescape($value[1]) . $suffix;
                }
                else
                {
     ...
     * @return array
     */
    public function joinAll ($where=array (), $orderByArr=array (), $limitArr=array (), $fileds=array(),$joinStr=""){
        $field = isset($fileds) ? implode(',', $fileds) : '*';
        $row_count = 0;
        $offset = 0;
        $sql = 'SELECT '.$this->formatFields($field).' FROM '.$this->_tableName
            .$joinStr
...
//        echo $sql;die();
     ...
     *
     * */
    public function joinOne ($fieldArr, $whereArr, $orderByArr = [],$joinStr=""){
        $sql = 'SELECT '.$this->formatFields($fieldArr).' FROM '.$this->_tableName .$joinStr. $this->formatWhere($whereArr) . $this->formatOrderBy($orderByArr);
//        echo $sql;die();
        return Core_Db::fetchOne($sql);
    }
