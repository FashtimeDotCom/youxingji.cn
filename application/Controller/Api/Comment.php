...
//      }
    ...
    public function get_comment_page_listAction()
    {
        /*if (!$this->isPost()) {
            $json = array('status' => 1, 'msg' => '非法操作!');
            echo Core_Fun::outputjson($json);
            exit;
        }*/
        if (!$this->user) {
            $json = array('status' => 0, 'is_login' => 0);
            echo Core_Fun::outputjson($json);
        }
        $faq_id = $this->getParam('id');
        $pid = $this->getParam('pid');
        $type = $this->getParam('type') ?? 4;
        ...
        $son = C::M('comment as a')->field("a.*,b.username,c.username as to_username,b.realname,c.realname as to_realname,b.headpic")
            ->joins($joins)
            ->where("rid={$faq_id} and type={$type} and pid={$pid}")->limit($limit)->order('id ASC')->select();
        if ($son) {
            foreach ($son as $k => $v) {
                $son[$k]['headpic'] = empty($v['headpic']) ? 'resource/images/img-lb2.png' : $v['headpic'];
                $son[$k]['addtime'] = date('Y-m-d H:i', $v['addtime']);
            }
            $json = array('status' => 0, 'data' => $son);
            echo Core_Fun::outputjson($json);
        } else {
            $json = array('status' => 1, 'msg' => '暂无更多');
            echo Core_Fun::outputjson($json);
        }
    }
}