 ...
 * Time: 14:50
 */
class Controller_Admin_SketchEveryday extends Core_Controller_Action
{
    var $_pagetitle="详细行程";
    ...
    public function indexAction()
    {
        ...
        $type_model=new Model_SketchEveryday();
        $perpage=15;
        $mapurl="admin/sketchEveryday/index/";
        ...
        $this->display("admin/sketch/sketch_everyday.tpl");
    }
    ...
    public function editAction($id=0)
    {
        ...
        //获取写生列表
        $sketch_list=C::M("sketch")->field("id,title")->where(" status=1 or status=0 ")->select();
        if( $sketch_list ){
        ...
        $this->display("admin/sketch/sketch_everyday_edit.tpl");
    ...
    public function saveAction()
    {
        ...
        if( $id>0 ){//update
            $res=C::M("sketch_everyday")->where(" id = {$id}")->update($data);
            if( $res>0 ){
            ...
            $res=C::M("sketch_everyday")->add($data);
            if( $res>0 ){
                echo json_encode(array("code"=>1,"message"=>"success"));
                exit();
            }else{
...
* */
    public function update_statusAction()
    {
        ...
        if( $id >0 ){
            $res=C::M("sketch_everyday")->where(" id={$id} ")->delete();
            if( $res>0 ){
                echo json_encode(array("code"=>1,"message"=>"success"));
                exit();
            }else{
