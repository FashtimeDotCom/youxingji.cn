 ...
 * @author: snake.L
 */
class Controller_Admin_Category extends Core_Controller_Action
{
	//分类Model.
		...
		if ($this->getParam ('newcatname')) {
            ...
            foreach ($_newparentids as $i => $_newparentid)
            {
                $_cat = array (
                    ...
                    'sort' => htmlspecialchars($_newdisplayorders[$i]),
                    'useable' => 1
                );
        ...
        if ($this->getParam ('id')) {
            ...
            foreach ($_ids as $i => $_id)
            {
                $_cat = array (
                    ...
                    'sort' => htmlspecialchars($_displayorders[$_id]),
                    'useable' => $_names[$_id] && isset ($_useables[$_id]) && $_useables[$_id]
                );
	...
	public function editAction()
	{
		...
		else
		{
			...
			$this->assign('content', Core_Fun::getEditor('content', $category['content'], 200, 700, 'bbs'));
