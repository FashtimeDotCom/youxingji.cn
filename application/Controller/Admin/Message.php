 ...
 * @author Snake.L
 */
class Controller_Admin_Message extends Core_Controller_Action
{
	...
	public function __construct($params)
	{
		...
		$this->_messageModel = new Model_User_Message();
	}
	...
	public function sendAction()
	{
		...
		if($action && $action == 'message')
		{
			...
			if(!empty($uid))
			{
				$userid = Core_Db::getOne("SELECT uid FROM ##__user_member WHERE usersn LIKE '%$uid%' OR username LIKE '%$uid%'");
				$messagedata = array(
					'type' => 'system',
					'fromuid' => 1,
					'touid' => $userid,
