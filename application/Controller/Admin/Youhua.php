...
class Controller_Admin_Youhua extends Core_Controller_Action
{
	...
	public function morewriterAction()
	{
        if($this->getParam('id'))
		{
            ...
            $_isfamous=$this->getParam("famous");
  	...
  	public function edittvAction()
	{
		...
		if($action && $action == 'edit')
		{
			$data = array(
				...
				'url' => $this->getParam('url'), 
				'describes' => $this->getParam('describes'),
                'addtime'=>strtotime($this->getParam('addtime'))
			);
		...
		{
		    $writer['addtime']=date('Y-m-d H:i:s',$writer['addtime']);
			$this->assign('picdom', 'imgurl');
			$this->assign('article', $writer);
	...
	public function star_editAction(){
        $id=$this->getParam("id")?$this->getParam("id"):0;
        if( $id ){
            //
            $sql="SELECT a.*,b.username FROM ##__travel as a LEFT JOIN ##__user_member as b on b.uid=a.uid WHERE a.id={$id}";
        ...
        $this->display('admin/youhua/star_edit.tpl');
    }

    public function star_saveAction(){
        $id=$this->getParam('id');
        $addtime=$this->getParam('addtime');
        if( $id ){
            $id=intval($id);
            ...
            $res=C::M("travel")->where("id={$id}")->update($data);
            if( $res ){
                echo json_encode(array("code"=>1,"message"=>"success"));
                exit();
            }else{
                echo json_encode(array("code"=>0,"message"=>"修改失败"));
                exit();
            }
