 ...
 * Time: 14:02
 */
class Controller_Admin_SketchDetail extends Core_Controller_Action
{
    var $_pagetitle="写生详情";
    ...
    public function indexAction()
    {
        ...
        $type_model=new Model_SketchDetail();
        $perpage=15;
        $mapurl="admin/sketchDetail/index/";
        ...
        $this->display("admin/sketch/sketch_detail.tpl");
    }
    ...
    public function editAction($id=0)
    {
        ...
        $this->display("admin/sketch/sketch_detail_edit.tpl");
    }
    ...
    public function saveAction()
    {
        ...
        if( $id>0 ){//update
            $res=C::M("sketch_detail")->where(" id = {$id}")->update($data);
            if( $res>0 ){
            ...
            $res=C::M("sketch_detail")->add($data);
            if( $res>0 ){
                echo json_encode(array("code"=>1,"message"=>"success"));
                exit();
            }else{
...
* */
    public function update_statusAction()
    {
        ...
        if( $id >0 ){
            $res=C::M("sketch_detail")->where(" id={$id} ")->delete();
            if( $res>0 ){
                echo json_encode(array("code"=>1,"message"=>"success"));
                exit();
            }else{
