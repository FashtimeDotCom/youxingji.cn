 ...
 * @author: Snake.L
 */
class Controller_Admin_Forum extends Core_Controller_Action
{
	//分类Model.
    ...
    public function __construct($params)
    {
        ...
        $this->_forumModel = new Model_Forum();
		$this->_threadModel = new Model_Forumthread();
		$this->_postModel = new Model_Forumpost();
    }
	...
	public function editAction()
	{
		...
		if($action && $action == 'edit')
		{
			...
			{
				Core_Fun_File::delete(BASEROOT . $forum['forumpic']);
		...
		else
		{
			...
			$this->assign('forumsummary', Core_Fun::getEditor('forumsummary', $forum['forumsummary'], 200, 700, 'bbs'));
	...
	public function threadaddAction()
	{
		...
		if($action && $action == 'threadadd')
		{
			...
			$data = array(
				'forumid' => $forumid,
				'realname' => Core_Config::get('site_name', 'basic', ''),
				'uid' => 1,
				'title' => $this->getParam('title'),
				'addtime' => $time,
				'lastpost' => $time,
				'lastpostuid' => 1,
				'lastposter' => Core_Config::get('site_name', 'basic', ''),
				'views' => 1,
				'useable' => 1,
				'replies' => 0
			);
		...
		else
		{
			...
			$this->assign('forums', $this->_forumModel->getForumTree());
			$this->assign('content', Core_Fun::getEditor('content', '', 200, 797, 'bbs'));
	...
	public function threadeditAction()
	{
		...
		if($action && $action == 'threadedit')
		{
			...
			{
				Core_Fun_File::delete(BASEROOT . $thread['attach']);
				$data['attach'] = $attach;
			}
			else
			{
		...
		else
		{
			$this->assign('picdom', 'attach');
			$this->assign('forums', $this->_forumModel->getForumTree());
			$this->assign('content', Core_Fun::getEditor('content', $thread['content'], 200, 797, 'bbs'));
