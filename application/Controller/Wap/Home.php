 ...
 * 手机首页
 */
class Controller_Wap_Home extends Core_Controller_WapAction
{
	private $_cUser;
    ...
    public function orderAction()
    {
    	...
    	$orders = C::M('order')->queryAll($where, 'addtime DESC');
    }
	...
	public function passwordAction()
	{
		...
		elseif(! C::M('User_Member')->checkUserOldPassword($user['username'], $oldpwd))
		{
			$arr['msg'] = 0;
			$arr['html'] = '旧密码不正确';
		}
		else
		{
			...
			$conditions['password'] = C::M('User_Member')->formatPassword($password, $conditions['salt']);
			//将密码更新到数据库
	...
	public function telephoneAction()
	{
		...
		$valid = C::M('Base_Valid')->getValidInfoByTelephoneAndType($telephone, 'new');
	...
	public function organAction()
	{
		...
		if($action && $action == 'organ')
		{
			...
			$geo = Core_Comm_Area::getLngLat('北京市' . $district . $road . $address);
			...
			{
				$fileUrl = Core_Util_Upload::upload('organpic', '/organ', 'jpg,jpeg,png,gif,JPG,JPEG', false, false, false);
				$data['organpic'] = $fileUrl['link'];			
			}
			if('' != $_FILES['markpic']['tmp_name'])
			{
				$fileUrl = Core_Util_Upload::upload('markpic', '/mark', 'jpg,jpeg,png,gif,JPG,JPEG', false, false, false);
				$data['markpic'] = $fileUrl['link'];			
			}
			if('' != $_FILES['qrcode']['tmp_name'])
			{
				$fileUrl = Core_Util_Upload::upload('qrcode', '/qrcode', 'jpg,jpeg,png,gif,JPG,JPEG', false, false, false);
				$data['qrcode'] = $fileUrl['link'];			
			}
			if(C::M('organ')->addOrgan($data))
			{
		...
		{
			$organ = C::M('organ')->getOrganByUid($uid);
	...
	public function courseAction()
	{
		...
		{
			$orders[$idx]['organname'] = C::M('organ')->getOrganIdByUid($order['uid'], 'organname');
			$orders[$idx]['location'] = Core_Comm_Util::getLocation('', '', $order['district'], $order['road']);
		}
	...
	public function feedAction()
	{
        ...
        $multipage = $this->multipage ($Num, $perpage, $curpage, $mpurl);
		$feeds = C::M('feedback')->queryAll($where, 'addtime DESC', array($perpage, $perpage * ($curpage - 1)));
	...
	public function dfeedAction()
	{
		...
		foreach($orders AS $idx => $order)
		{
			if(!C::M('feedback')->isFeedBack($order['uid'], $order['goodsid'], $order['buytype']))
			{
				if($order['buytype'] == 'course')
				{
					$good = C::M('course')->getCourseById($order['goodsid']);
					$good['title'] = $good['coursename'];
					$good['pic'] = $good['coursepic'];
				}
				else
				{
					$good = C::M('event')->getEventById($order['goodsid']);
	...
	public function payAction()
	{
        ...
        $multipage = $this->multipage ($Num, $perpage, $curpage, $mpurl);
		$orders = C::M('order')->queryAll($where, 'addtime DESC', array($perpage, $perpage * ($curpage - 1)));
		$this->assign('multipage', $multipage);
		$this->assign('orders', $orders);
	...
	public function stowAction()
	{
        ...
        $multipage = $this->multipage ($Num, $perpage, $curpage, $mpurl);
		$stows = C::M('stow')->queryAll($where, 'addtime DESC', array($perpage, $perpage * ($curpage - 1)));
		foreach($stows AS $idx => $stow)
		{
			$headpic = C::M('User_Member')->getInfoByUid('headpic50', $stow['uid']);
			$realname = C::M('User_Member')->getInfoByUid('realname', $stow['uid']);
			$headpic = $headpic ? $headpic : Core_Fun::getPathroot() . 'resource/images/user_head_img.gif';
			$realname = $realname ? $realname : '匿名';
