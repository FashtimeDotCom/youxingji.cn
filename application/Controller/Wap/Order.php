 ...
 * 购买控制器
 */
class Controller_Wap_Order extends Core_Controller_WapAction
{
	...
	public function buyAction()
	{
		...
		$user = $this->getUser();
		if(!$user['uid'])
			$this->showmsg('', 'wap/u/login?refer=' . Core_Fun::iurlencode(Core_Fun::getPathroot() . 'wap/order/buy/id/' . $id . '/buytype/' . $buytype), 0);
		if(!$id || !$buytype)
			$this->showmsg('不要提交未知订单', 'wap');
		if($buytype == 'course')
		{
			$cart = C::M('course')->find($id);
			$cart['title'] = $cart['coursename'];
		}
		else
		{
			$cart = C::M('event')->find($id);
	...
	public function payAction()
	{
		...
		$order = C::M('order')->getOrderByOrderSn($ordersn);
		if(!$order['id'])
	...
	public function notifyAction()
	{
		...
		if($notify->checkSign() == TRUE)
		{
			if ($notify->data["return_code"] == "SUCCESS")
			{
				...
				$order = C::M('order')->getOrderByOrderSn($out_trade_no);
				if(!$order['id'])
					exit;
				if($order['state'] == 1)
					exit;
				...
				C::M('order')->updateByOrderSn($data);
			}
		}
	}
	...
	public function successAction()
	{
		$ordersn = $this->getParam('ordersn');
		$order = C::M('order')->getOrderByOrderSn($ordersn);
		/*$telephone = C::M('order')->getTelephoneByOrganId($order['organid']);
	...
	public function bookingAction()
	{
		...
		$week = Core_Fun::date('w', $order['booktime']);
		$order['week'] = $weeks[$week];
		if($step == 3)
		{
			$cat = C::M('category')->find($order['catid']);
		...
		elseif($step == 4)
		{
			$this->assign('calendar', Core_Comm_Calendarwap::display('', '', $orderid));
			$this->assign('shopname', $order['shopname']);
		}
		elseif($step == 5)
		{
			$this->assign('timestr', Core_Comm_Calendarwap::displayTime($order['shopid']));
		}
		$this->assign('order', $order);
	...
	public function doneAction()
	{
		...
		if($step == 1)
		{
			$ordersn = Core_Fun::getOrdersn('wm');
			$shopid = $this->getParam('shopid');
			$shopname = C::M('shop')->getOneById('shopname', $shopid);
			$data = array(
				'ordersn' => $ordersn,
				'shopid' => $shopid,
				'shopname' => $shopname,
		...
		elseif($step == 2)
		{
			...
			$price = C::M('category')->getOneById('price', $catid);
			$data = array(
				'id' => $orderid,
				'catid' => $catid,
				'price' => $price,
		...
		{
			$orderid = $this->getParam('orderid');
			$ordersn = C::M('order')->getOneById('ordersn', $orderid);
