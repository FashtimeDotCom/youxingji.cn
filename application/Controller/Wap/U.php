 ...
 * Snake.L
 */
class Controller_Wap_U extends Core_Controller_WapAction
{
	private $userModel;
	...
	public function doregAction()
	{
		...
		$valid = C::M('Base_Valid')->getValidInfoByTelephoneAndType($username, 'reg');
		...
		if($sign && $uvalid && $sign == $uvalid)
		{
			...
			if ($uid) 
			{
				C::M('Base_Valid')->updateValidByTelephoneAndType($username, 'reg');
				//设置会话保持
				...
				$userInfo['regip'] = Core_Comm_Util::getClientIp();
				$userInfo['regtime'] = Core_Fun::time();
				$this->userModel->editUserInfo($userInfo);
				$this->userModel->onSetCurrentUser($uid, $userInfo['username'], $roleid);
				if($roleid == 2)
				{
					C::M('organ')->addOrgan(array(
						'uid' => $uid,
						'useable' => 0,
						'addtime' => Core_Fun::time()
					));
					$this->showmsg('注册成功，请耐心等待哈哈乐学审核通过', 'wap/home');
				}
				else
				{
					$this->showmsg('', 'wap/home', 0);
	...
	public function loginAction()
	{
		...
		{
			$referurl = Core_Fun::iurldecode($this->getParam('refer'));
	...
	public function dologinAction()
	{
		...
		{
            $this->setCookie('vpcv_token', Core_Comm_Token::authcode($user['uid'] . "\t" . $sign, 'ENCODE'), 3600 * 24 * 30);
		}
	...
	public function findAction()
	{
		...
		{
			if(Core_Config::get('open', 'mail', false))
			{
				$username = $this->getParam('username');
				$email = $username;
				//用户名为空
				...
				//发送邮件
				$url = Core_Fun::getPathroot() . 'u/changepwd?uid='.$user['uid'].'&sign='.$tokenSign;
	...
	public function qqAction()
	{
		...
		Core_Outapi_Qc::setAppid();
		Core_Outapi_Qc::setAppkey();
		Core_Outapi_Qc::setCallback($callback);
		$url = Core_Outapi_Qc::LoginUrl();
		$this->showmsg('', $url, 0);
	}
	...
	public function qqbindAction()
	{
		...
		Core_Outapi_Qc::setAppkey();
		Core_Outapi_Qc::setCallback($callback);
		$token = Core_Outapi_Qc::AccessToken($code);
		$openid = Core_Outapi_Qc::GetOpenid($token);
		if(!empty($openid))
		{
			...
			{
				$openuser = Core_Outapi_Qc::GetUserInfo($openid, $token);
	...
	public function ajaxcheckuserAction()
	{
		...
		echo Core_Fun::array2json(array('msg' => $msg));
	}
	...
	public function dofindAction()
	{
		...
		$uvalid = strtolower($uvalid);
		$valid = C::M('Base_Valid')->getValidInfoByTelephoneAndType($username, 'find');
		$sign = strtolower($valid['sign']);
		if($sign && $uvalid && $sign == $uvalid)
		{
			$uid = C::M('User_Member')->getInfoByUsername('uid', $username);
			...
			{
				C::M('Base_Valid')->updateValidByTelephoneAndType($username, 'find');
