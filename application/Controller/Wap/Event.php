 ...
 * 活动控制器
 */
class Controller_Wap_Event extends Core_Controller_WapAction
{
	...
	public function indexAction()
    {
		...
		if($kind)
		{
			...
			$this->assign('catname', C::M('category')->getCatNameByCatId($kind));
		}
		if($road)
		{
		...
		$events = C::M('event')->queryAll($where, $_orderby, array($perpage, $perpage * ($curpage - 1)), array('*', 'GETDISTANCE(lat,lng,'.$mylat.','.$mylng.') AS km'));
	...
	{
		$id = $this->getParam('id');
		$event = C::M('event')->find($id);
		if(!$event['id'] || !$event['useable'])
			$this->showmsg('该活动不存在或者正在审核中', 'wap/event');
		C::M('event')->updateNum($id, 'shownum');
		$event = $this->formatHourMinute($event);
		$event['location'] = Core_Comm_Util::getLocation('', '', $event['district'], $event['road']);
		$event['organname'] = C::M('organ')->getOrganIdByUid($event['uid'], 'organname');
		$this->assign('event', $event);
		$this->assign('curr', '活动详情');
		$this->assign('stowid', $id);
		$this->assign('stowtype', 'event');
		$this->assign('stowtitle', $event['eventname']);
		$this->assign('orders', C::M('order')->getOrderList('event', $id));
	...
	public function feedAction()
	{
		...
		$user = $this->getUser();
		if(!$user['uid'])
			$this->showmsg('', 'wap/u/login?refer=' . Core_Fun::iurlencode(Core_Fun::getPathroot() . 'wap/event/feed/id/' . $id), 0);
		$eventname = C::M('event')->getOneById('eventname', $id);
		$this->assign('id', $id);
