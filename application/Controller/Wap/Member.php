 ...
 * 手机首页
 */
class Controller_Wap_Member extends Core_Controller_WapAction
{
	private $_cUser;
	...
	{
		$uid = $this->_cUser['uid'];
		$organid = C::M('organ')->getOrganIdByUid($uid);
		if(!C::M('organ')->isCheck($organid))
		...
		if($action && $action == 'course')
		{
			...
			$endtime = Core_Fun::strtotime($timeyear . '-' . $timemonth . '-' . $timeday);
			$enddate = Core_Fun::strtotime($endyear . '-' . $endmonth . '-' . $endday);
			...
			{
				$fileUrl = Core_Util_Upload::upload('coursepic', '/course', 'jpg,png,gif,JPG,JPEG', false, false, false);
				$data['coursepic'] = $fileUrl['link'];
			}
			if('' != $_FILES['markpic']['tmp_name'])
			{
				$fileUrl = Core_Util_Upload::upload('markpic', '/mark', 'jpg,png,gif,JPG,JPEG', false, false, false);
				$data['markpic'] = $fileUrl['link'];
			}
			
			if(C::M('course')->add($data))
			{
		...
		else
		{
			...
			$catlist = C::M('category')->getCategoryTree(0, 2, '', '--');
	...
	public function eventAction()
	{
		...
		if($action && $action == 'event')
		{
			...
			$begintime = Core_Fun::strtotime($eventyear . '-' . $eventmonth . '-' . $eventday);
			$endyear = $this->getParam('endyear');
			$endmonth = $this->getParam('endmonth');
			$endday = $this->getParam('endday');
			$enddate = Core_Fun::strtotime($endyear . '-' . $endmonth . '-' . $endday);
			...
			{
				$fileUrl = Core_Util_Upload::upload('eventpic', '/event', 'jpg,png,gif,JPG,JPEG', false, false, false);
		...
		else
		{
			...
			$catlist = C::M('category')->getCategoryTree(0, 2, '', '--');
			$this->assign('catlist', $catlist);
			$kindlist = C::M('category')->getCategoryTree(0, 3, '', '--');
	...
	public function activeAction()
	{
		...
		if($action && $action == 'active')
		{
			...
			{
				$fileUrl = Core_Util_Upload::upload('activepic', '/active', 'jpg,jpeg,png,gif,JPG,JPEG', false, false, false);
				$data['activepic'] = $fileUrl['link'];
			}
			
			if(C::M('active')->add($data))
			{
	...
	public function headpicAction()
	{
		...
		{
			$type = $result[2];
			$new_file = Core_Fun::date("YmdHis") . mt_rand(1000,9999) . '.' . $type;
			if(!file_exists(HTDOCS_ROOT . 'uploadfile/headpic'))
			{
				Core_Fun_File::makeDir(HTDOCS_ROOT . 'uploadfile/headpic');
			...
			if (file_put_contents(HTDOCS_ROOT . 'uploadfile/headpic/' . $new_file, base64_decode(str_replace($result[1], '', $imgurl)))){
				$headpic = Core_Fun::getWebroot() . 'uploadfile/headpic/' . $new_file;
			}
		}
		else
		{
	...
	public function spicAction()
	{
		...
		{
			$fileUrl = Core_Util_Upload::upload('spic', '/headpic', 'jpg,jpeg,JPG,JPEG,png,gif', false, true, true);
			$ret['headpic'] = $fileUrl['link'];
			$ret['imgurl'] = $fileUrl['thumb'];
			$ret['msg'] = 1;
		}
		echo Core_Fun::array2json($ret);
	}
