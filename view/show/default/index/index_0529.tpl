        ...
        function getryt(time,t) {
            $.ajax({
                ...
                success: function(data){
                    if(data['id']){
                        $('#getryt').html('<div style="height: 460px;overflow: hidden;"><h5>'+data['title']+'</h5><div class="date">'+time+'</div>'+data['homecontent']+'</div><a href="/index.php?m=index&c=index&v=rytdetai&id='+data['id']+'" class="more" target="_blank">阅读全文</a>');
                    }else{
                        $('#getryt').html("无内容");
                    }
                }
