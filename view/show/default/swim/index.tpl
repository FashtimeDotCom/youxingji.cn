			...
			<div class="item" style="background: url({{$item.img_url}}) no-repeat center center;background-size:100% 100%;">
				<div class="box">
					...
					<div class="box-user">
						...
						<div class="user-info">
							<div class="user-head">
								<div class="Head_portrait">
									<img src="{{$item.info.headpic}}"/> <!--暂时写死-->
								</div>
								<div class="user_name">
									<h1>种子选手</h1><!--暂时写死-->
									<h1>{{$item.info.username}}</h1><!--暂时写死-->
								</div>
							</div>
							<div class="user-text">
								{{$item.info.describes}}
							...
							<ul class="row">
								{{foreach from=$item.info.content item=val key=k}}
									<li><a><img src="http://www.youxingji.cn{{$val}}"/></a></li>
								{{/foreach}}
						...
						<input type="button" name="live" data-url="{{if $item.living}}/index.php?m=index&c=swimdetail&v=detail&act_id={{$item.living.activity_id}}&tar_id={{$item.living.target_id}}&u={{$item.user_id}}{{else}}javascript:void(0){{/if}}" class="live" id="live" value="更多直播" style="background-image: url('/resource/images/swim/icon_xm.png');" /><!--渲染三级页面后改-->
					</div>
