...
<div class="middle_zone">
    ...
    <button class="btn btn-success pull-right" style="margin-top: 30px;margin-right: 30px;background-color:#60C060;"
            onclick="Controller.main('{{$_pathroot}}admin/starTravelEveryday/add')">新增</button>
...
{{if $list}}
<form id="lpform" name="lpform" method="post" action="/admin/starTravelEveryday/index">
    <table class="tb tb2 table table-striped">
        ...
        <tr>
            ...
            <td class=" td-operation">
                <a class="btn btn-success btn-sm item" href="javascript:;" onclick="Controller.main('{{$_pathroot}}admin/starTravelEveryday/edit/id/{{$modules.id}}')">编辑</a>
                <a class="btn btn-danger btn-sm" href="javascript:;" onclick="update_status({{$modules.id}})">删除 </a>
            </td>
        </tr>
        {{/foreach}}
...
<script>
    function update_status(id)
    {
        ...
        layer.confirm('您确定要删除吗？', {
            ...
            $.post("{{$_pathroot}}admin/starTravelEveryday/update_status",{id:id}, function(data) {
                if(!data.error){
