...
<div class="middle_zone">
    ...
    <button class="btn btn-success pull-right" style="margin-top: 30px;margin-right: 30px;background-color:#60C060;"
            onclick="Controller.main('{{$_pathroot}}admin/starTravel/add')">新增</button>
...
{{if $list}}
<form id="lpform" name="lpform" method="post" action="/admin/starTravel/index">
    <table class="tb tb2 table table-striped">
        ...
        <tr>
            ...
            <td>{{$modules.title}}</td>
            <td>{{$modules.days}}</td>
            <td>{{$modules.depature_time}}</td>
            <td>{{$modules.price}}</td>
            <td>{{$modules.status}}</td>
            <td>{{$modules.username}}</td>
            <td class=" td-operation">
                <a class="btn btn-success btn-sm item" href="javascript:;" onclick="Controller.main('{{$_pathroot}}admin/starTravel/edit/id/{{$modules.id}}')">编辑</a>
...
<script>
    ...
    {
        $.post("{{$_pathroot}}admin/starTravel/update_status",{id:id}, function(data) {
            if(!data.error){
