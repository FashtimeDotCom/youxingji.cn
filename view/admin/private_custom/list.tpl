...
<div class="middle_zone">
    ...
    <ol class="breadcrumb" style="display: inline-block;border: none;">
        ...
        <li><a href="javascript:void(0)">独家旅行</a> / </li>
...
<form id="lpform" name="lpform" method="post" action="/admin/sketchEveryday/index">
    <table class="tb tb2 table table-striped">
        ...
        <tr>
            ...
            <td>{{$modules.mobile}}</td>
            <td>{{$modules.status}}</td>
            <td>{{$modules.add_time}}</td>
            <td>
                <a class="btn btn-success btn-sm item" href="javascript:;" onclick="Controller.main('{{$_pathroot}}admin/PrivateCustom/edit/id/{{$modules.id}}')">编辑</a>
            </td>
        </tr>
        {{/foreach}}
