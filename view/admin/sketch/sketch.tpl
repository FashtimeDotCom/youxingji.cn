...
<div class="middle_zone">
    ...
    <button class="btn btn-success pull-right" style="margin-top: 30px;margin-right: 30px;background-color:#60C060;"
            onclick="Controller.main('{{$_pathroot}}admin/sketch/add')">新增</button>
...
{{if $list}}
<form id="lpform" name="lpform" method="post" action="/admin/sketch/index">
    <table class="tb tb2 table table-striped">
        ...
        <tr>
            ...
            <td class=" td-operation">
                <a class="btn btn-success btn-sm item" href="javascript:;" onclick="Controller.main('{{$_pathroot}}admin/sketch/edit/id/{{$modules.id}}')">编辑</a>
...
<script>
    ...
    {
        $.post("{{$_pathroot}}admin/sketch/update_status",{id:id}, function(data) {
            if(!data.error){
