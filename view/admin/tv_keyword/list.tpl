...
<div class="middle_zone">
    ...
    <button class="btn btn-success pull-right" style="margin-top: 30px;margin-right: 30px;background-color:#60C060;"
            onclick="Controller.main('{{$_pathroot}}admin/TvKeyword/add')">新增</button>
...
<form id="lpform" name="lpform" method="post" action="/admin/sketchEveryday/index">
    <table class="tb tb2 table table-striped">
        ...
        <tr>
            <td>{{$modules.id}}</td>
            <td>{{$modules.title}}</td>
            <td><img src="{{$modules.thumbfile}}" width="100"></td>
            <td>{{$modules.keyword}}</td>
            <td>{{$modules.status}}</td>
            <td>
                <a class="btn btn-success btn-sm item" href="javascript:;" onclick="Controller.main('{{$_pathroot}}admin/TvKeyword/edit/id/{{$modules.id}}')">编辑</a>
...
<script>
    function delete_log(id)
    {
        ...
        layer.confirm('你确定要删除吗?', {
            ...
            $.post("{{$_pathroot}}admin/TvKeyword/delete",{id:id}, function(data) {
                if(!data.error){
