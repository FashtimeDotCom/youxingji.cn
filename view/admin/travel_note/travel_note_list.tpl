...
<form id="lpform" name="lpform" method="post" action="/admin/TravelNote/index">
    <table class="tb tb2 table table-striped">
        ...
        <tr>
            ...
            <td>{{$modules.show_num}}</td>
            <td>{{$modules.top_num}}</td>
            <td>{{$modules.is_top}}</td>
            <td>{{$modules.username}}</td>
            <td class=" td-operation">
                <a class="btn btn-success btn-sm item" href="javascript:;" onclick="Controller.main('{{$_pathroot}}admin/TravelNote/edit/id/{{$modules.id}}')">编辑</a>
...
<script>
    ...
    {
        $.post("{{$_pathroot}}admin/TravelNote/update_status",{id:id}, function(data) {
            if(!data.error){
